...
### FR-15 Modalita turno attivo

- Il sistema deve supportare una modalita Turno attivo con contesto persistente (residenza, turno, progressione paziente n/N).
- In modalita Turno attivo deve essere disponibile la navigazione rapida al prossimo paziente in sequenza.

### FR-16 Correzione evento tracciata

- Le correzioni di esiti gia registrati devono essere consentite solo ad admin.
- Ogni correzione deve richiedere motivazione obbligatoria e produrre audit completo (valore precedente, nuovo valore, attore, timestamp).

### FR-17 Gestione conflitti visiva

- Il sistema deve mostrare i conflitti di sincronizzazione in una vista operativa comprensibile (locale vs remoto vs decisione scelta).
- L'utente deve poter risolvere i conflitti tramite azioni guidate con audit automatico.

### FR-18 Handover strutturato di fine turno

- Il sistema deve generare una consegna strutturata di fine turno con checklist obbligatoria.
- La consegna deve includere almeno: non somministrati, rifiuti, assenti, alert scorte aperti, note critiche.

### FR-19 Ownership alert scorte

- Ogni alert scorte deve avere assegnatario, stato (Aperto, Preso in carico, Risolto) e timestamp di presa in carico/chiusura.
- La chiusura alert deve restare vincolata a un movimento di magazzino tracciato.

### FR-20 Reportistica stampabile (Node web)

- La piattaforma Node deve supportare report operativi stampabili da browser (layout print-friendly A4).
- I report minimi MVP includono: riepilogo turno somministrazioni, alert scorte aperti/chiusi, movimenti magazzino per periodo.
- Ogni report stampato deve mostrare metadata minimi: data/ora generazione, filtro applicato, operatore che ha generato il report.
...
### NFR-11 Affidabilita UX offline

- Le azioni critiche devono essere eseguibili anche offline con massimo un tap aggiuntivo rispetto al flusso online.
- La coda eventi offline deve essere consultabile lato utente con stato per singolo evento.

### NFR-12 Comprensibilita stato sistema

- Lo stato sync deve essere interpretabile da un operatore non tecnico in meno di 2 secondi.
- I messaggi di stato devono essere azionabili (es. In coda, Riprova ora, Richiede attenzione).

### NFR-13 Tracciabilita correzioni

- Ogni modifica post-evento deve essere ricostruibile integralmente a fini clinici/ispettivi.
- Il sistema deve conservare storico immutabile delle versioni evento (old/new + motivo + attore + timestamp).
...
- Reportistica operativa stampabile (print-friendly) per somministrazioni, scorte e movimenti.
...
6. Reportistica stampabile lato Node per supervisione e audit operativo.
7. Test E2E cross-client e hardening sicurezza.
...
#### F) Reportistica stampabile

Obiettivo: fornire report operativi pronti alla stampa per supervisione clinico-logistica.

Struttura componenti:
- Filtri report: periodo, residenza, reparto, operatore, stato evento.
- Selettore tipo report:
  - Riepilogo somministrazioni turno
  - Alert scorte (aperti/chiusi)
  - Movimenti magazzino per periodo
- Anteprima report in pagina (layout A4).
- Azioni:
  - Stampa
  - Esporta PDF

Copy UI:
- Titolo pagina: Reportistica operativa
- Label filtro: Periodo report
- Label filtro: Residenza
- Label filtro: Reparto
- CTA: Genera report
- CTA: Stampa
- CTA: Esporta PDF
- Footer report: Generato il {data_ora} da {operatore}

Regole anti errore:
- Stampabili solo report coerenti con i filtri selezionati.
- Ogni generazione report deve essere tracciata in audit.
...
## 12) Miglioramenti operativi implementabili (MVP -> scalabile)

### 12.1 Quick wins (impatto alto, effort basso)

- Modalita Turno attivo con contesto fisso (residenza/turno/progresso) e CTA principali sempre visibili.
- Undo intelligente a due livelli: undo rapido (10s) + correzione admin tracciata.
- Focus mode somministrazione con vista essenziale (ospite, farmaco, dose, alert, esito).
- Stato sync umanizzato con messaggi operativi (Registrato, In coda, Richiede attenzione).
- Microcopy orientata all'azione per ridurre esitazione in reparto.

### 12.2 Miglioramenti sicurezza clinica

- Guardrail dose/frequenza in tempo reale su incoerenze rispetto al piano terapeutico.
- Pattern Stop-Look-Act per alert critici con checklist minima prima della registrazione.
- Correzioni post-evento con doppia traccia (originale/corretto) e motivo obbligatorio.
- Verifica barcode/QR come secondo canale opzionale in fase successiva (non bloccante per MVP).

### 12.3 Miglioramenti offline/sync

- Vista coda eventi con stato per singolo elemento (in coda, retry, conflitto, errore permanente).
- Gestione conflitti visuale e guidata (locale/remoto/decisione).
- Prioritizzazione sync per eventi critici (somministrazioni, alert scorte).
- Retry con countdown visibile e azione esplicita Riprova ora.

### 12.4 Miglioramenti cognitivi

- Modalita guidata step-by-step per operatori junior.
- Riduzione densita informativa dinamica (vista essenziale vs vista completa).
- Raggruppamento per stanza/letto con progress marker per ridurre context switching.
- Feedback multimodale (colore + icona + testo + vibrazione opzionale).

### 12.5 Miglioramenti magazzino

- Previsione fabbisogno breve (7/14 giorni) basata su consumi + terapie pianificate.
- Riconciliazione guidata per eccezioni con motivi precompilati.
- Soglie multilivello (warning, critica, blocco operativo per farmaci essenziali).
- Evidenza lotti in scadenza con priorita di utilizzo.

### 12.6 Miglioramenti collaborazione

- Handover strutturato turno-to-turno con checklist obbligatoria.
- Inbox operativa admin-operatori legata agli eventi per chiarimenti/correzioni.
- Ownership esplicita degli alert con SLA interno.
- Dashboard osservabilita operativa per ruolo (non solo log tecnico).

### 12.7 Priorita consigliata (Must / Should / Could)

Must:
- Modalita Turno attivo.
- Undo intelligente + correzione admin tracciata.
- Guardrail dose/frequenza.
- Gestione conflitti visiva.
- Stato sync umanizzato + coda eventi visibile.
- Handover strutturato.
- Ownership alert scorte.

Should:
- Focus mode somministrazione.
- Modalita guidata step-by-step.
- Soglie multilivello scorte.
- Riconciliazione guidata eccezioni.
- Dashboard osservabilita per ruolo.

Could:
- Previsione fabbisogno 7/14 giorni.
- Feedback multimodale avanzato.
- Verifica barcode/QR come secondo canale.

## 13) Gap residui rispetto a standard clinici e KPI operativi

### 13.1 Gap residui da pianificare

- Mancanza di doppia verifica assistita (secondo operatore o scansione) per farmaci ad alto rischio.
- Mancanza di protocollo strutturato di incident reporting in-app.
- Mancanza di KPI qualita clinica espliciti per reparto/turno.
- Mancanza di policy formalizzata per override in condizioni eccezionali.

### 13.2 KPI UX e operativi consigliati

- Tempo medio registrazione esito per paziente.
- Tocchi medi per somministrazione completata.
- Percentuale eventi registrati offline.
- Tempo medio convergenza sync per eventi critici.
- Tasso correzioni post-evento (proxy qualita input).
- Percentuale alert scorte presi in carico entro SLA.
- Percentuale turni chiusi con handover completo.
- Numero conflitti per 100 eventi e tempo medio risoluzione.
- Tasso eventi con motivo mancante su rifiuto/assenza.
- Tempo medio da alert critico a presa in carico admin.

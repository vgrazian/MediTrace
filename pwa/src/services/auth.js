    ...
    if (EMERGENCY_ADMIN_ENABLED) {
        ...
        if (!state.hasUsers) {
            try {
                ...
                sessionToken = adminPayload.session?.token
                state.hasUsers = true
            } catch (err) {
                console.warn('[auth] Impossibile creare admin su Supabase:', err.message)
            }
        } else {
            // Try to get session token from stored session or admin login
            try {
                const payload = await restoreSession(AUTH_SESSION_TTL_MINUTES)
                sessionToken = payload?.session?.token || payload?.token
            } catch { /* no session yet */ }
        }

        // Create/update operators if we have a session token
        if (sessionToken) {
            const operators = [
            ...
            for (const op of operators) {
                try {
                    ...
                    if (rpcError && !String(rpcError.message || '').toLowerCase().includes('duplicate')) {
                        console.warn(`[auth] Operatore ${op.username}:`, rpcError.message)
                    }
                } catch (opErr) {
